				...
				if(rows.length!=1){
					...
					$.post("${pageContext.request.contextPath}/fixedAreaAction_findNotAssociation.action", null ,function(data){
						...
						for(var i=0;i<data.length;i++){
							var customerId = data[i].id;
							var name = data[i].username+'('+data[i].address+')'
							$("#noassociationSelect").append('<option value='+customerId+'>'+name+'</option>');
						}
					})
					$.post("${pageContext.request.contextPath}/fixedAreaAction_findHasAssociation.action", {"id":rows[0].id} ,function(data){
						...
						for(var i=0;i<data.length;i++){
							var customerId = data[i].id;
							var name = data[i].username+'('+data[i].address+')'
							$("#associationSelect").append('<option value='+customerId+'>'+name+'</option>');
						}
					})
			...
			var toolbar = [ {
				...
				handler : function(){
					...
					if(rows.length==1){
						...
						//通过js方式创建快递员记录下拉框
						$("#courierId").combobox({ 
								url:'${pageContext.request.contextPath}/courierAction_listajax.action?fixedAreaId='+fixedAreaId, 
								valueField:'id', 
								textField:'name' 
							});
						// 弹出定区关联快递员 窗口 
...
// 					url : "${pageContext.request.contextPath}/data/fixed_area.json",
	...
	<body class="easyui-layout" style="visibility:hidden;">
		...
		<!-- 关联客户窗口 /////////////////////////-->
		<div modal="true" class="easyui-window" title="关联客户窗口" id="customerWindow" collapsible="false" closed="true" minimizable="false" maximizable="false" style="top:20px;left:200px;width: 700px;height: 300px;">
			<div style="overflow:auto;padding:5px;" border="false">
							...
							<td colspan="3"><a id="associationBtn" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-save'">关联客户</a>
								<script type="text/javascript">
									...
									$("#toRight").click(function(){
										//目的：给右侧下拉框追加option(来源于左侧下拉框中选中option)
									...
									$("#associationBtn").click(function(){
										...
										//2、通过js代码给右侧select中option设置选中属性selected
